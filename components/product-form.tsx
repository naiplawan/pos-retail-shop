...
export default function PriceForm({ onDataChange }: PriceFormProps) {
  ...
  const handleBarcodeScan = (barcode: string) => {
    ...
    toast.success(`สแกนบาร์โค้ดเรียบร้อย: ${barcode}`);

    // Focus on product name field after scanning
  ...
  const handleBarcodeError = (error: string) => {
    toast.error(`ข้อผิดพลาดในการสแกน: ${error}`);
  };
  ...
  const handleAddToCart = () => {
    ...
    addToCart(values.productName, values.price, values.barcode);
  ...
  // Add an item to cart
  const addToCart = (productName: string, price: number, barcode?: string) => {
    setCartItems((prevCart) => {
      // Check if item already exists in cart (by barcode if available, otherwise by name)
      const existingItemIndex = prevCart.findIndex((item) =>
        barcode && item.barcode ? item.barcode === barcode : item.productName === productName
      );
      ...
      <div className="w-full md:w-[300px] p-6 border rounded-lg shadow-sm">
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div key={item.id} className="p-3 border rounded-md bg-muted/50">
                ...
                {item.barcode && <div className="text-xs text-muted-foreground mb-1">บาร์โค้ด: {item.barcode}</div>}
        ...
        {cartItems.length > 0 && (
          <div className="pt-3 border-t">
            ...
            <Button className="w-full" onClick={handleSaveAllCartItems} disabled={isSubmitting}>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value ? format(field.value, 'PPP', { locale: th }) : <span>เลือกวันที่</span>}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Button type="button" className="flex-1" variant="secondary" onClick={handleAddToCart}>
